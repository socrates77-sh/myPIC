...
handle_option (const char **argv, unsigned int lang_mask)
{
  ...
  if (opt_index == cl_options_count
    ...
    {
	  //fprintf(stderr,"opt_index:%d,cl_options_count:%d,opt:%s",opt_index ,cl_options_count,opt);
      /* Drop the "no-" from negative switches.  */
      size_t len = strlen (opt) - 3;

	   error ("----------------- \"%d\"", opt_index);
error ("----------------- \"%d\"", cl_options_count);
error ("----------------- \"%s\"", *opt);
  ...
  //error ("opt-------:%s",*option );
  //tflag=CL_COMMON ;
  //error ("CL_COMMON:%d",tflag );
  //tflag=CL_TARGET ;
  //error ("CL_TARGET:%d",tflag );
  //error ("option->flags:%d",option->flags );
  //   error ("lang_mask:%d",lang_mask );
  //tflag=(lang_mask | CL_COMMON | CL_TARGET) ;
  // error ("lang_mask | CL_COMMON | CL_TARGET:%d",tflag );
