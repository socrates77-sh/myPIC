...
DEBUGpic14_emitcode (char *inst, char *fmt, ...)
{
  ...
  if (!debug_verboseMC30 && !options.debug)
    return;
...
genJumpTab (iCode * ic)
{
  ...
  //wj modify emitpcode (POC_MOVLW, popGetHighLabel (jtab->key));
  //wj modify emitpcode (POC_MOVWF, popCopyReg (&pc_pclath));
  emitpcode (POC_MOVLW, popGetLabel (jtab->key));
  emitpcode (POC_ADDFW, popGet (AOP (IC_JTCOND (ic)), 0));
  //wj modify emitSKPNC;
  //wj modify emitpcode (POC_INCF, popCopyReg (&pc_pclath));
  emitpcode (POC_MOVWF, popCopyReg (&pc_pcl));
  emitpLabel (jtab->key);
