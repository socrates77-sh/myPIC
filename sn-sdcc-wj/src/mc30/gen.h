...
#define FENTRY do {                                                                     \
        ...
        if (options.debug || debug_verboseMC30) {                                           \
...
#define emitpcode(poc,pcop)     do { if (options.debug || debug_verboseMC30) { emitpComment (" >>> %s:%d:%s", __FILE__, __LINE__, __FUNCTION__); } emitpcode_real(poc,pcop); } while(0)
