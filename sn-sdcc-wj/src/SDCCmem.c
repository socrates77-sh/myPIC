...
void SaveGloabInfo(memmap * map, symbol * func,QValList *desvals)
{
	...
	symbol  *ev;
		   int testSize=0 ;
	if (!map || !map->syms)    return 0;
   

	for (sym = setFirstItem (map->syms); sym;
       sym = setNextItem (map->syms))
    {
      if (!((sym->level == 0) ||(sym->localof != func)))       continue;
...
void  SaveAllocInfo(symbol *func,QValList *desvals)
{
  ...
  if (isSavedGloab==0) isSavedGloab=1 ;
}
...
void printValsInfo(QValList *desvals, struct dbuf_s * oBuf)
{
	QValList* tempval ;
	symbol *sym ;
	if ((!desvals)||(!desvals->firstItem)) return ;
	else
